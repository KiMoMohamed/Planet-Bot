...
//=============================== - [ Bot ] - ===================================
 ...
 client.on('ready', () => {
      let guild = '468519385552650240';
    let channel = client.channels.get('490625651280445450')
    if(!channel) return;
    channel.send(`
    \`Bot Status :\` __Online__
    \`Logged in as :\` __${client.user.tag}__
    \`Servers :\`  __${client.guilds.size}__
    \`Users : \` __${client.users.size}__
    \`Channels :\` __${client.channels.size}__
    \`Developed By :\` <@${client.users.get("290908670529896448").id}>
`);
...
message.channel.send({embed:embed}).then(message => message.delete(5000));
  ...
  client.on('message',async message => {
    if(message.content.startsWith(prefix + "restart")) {
        if(message.author.id !== "290908670529896448") return;
        ...
        setTimeout(() => {
            client.destroy();
            client.login('NDY4NTE0MjczNDU5NTY4NjUy.DoBWMA.7mRvEPCdULfSCb1fZVpjLiUL3gA');
        },3000);
    }
});
